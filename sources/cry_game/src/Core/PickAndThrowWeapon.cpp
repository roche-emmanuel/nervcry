...
, m_logicFlags(ELogicFlag_None)
{
	...
	if(pickAndThrowActionHandler.GetNumHandlers() == 0)
	{
		...
		{
			pickAndThrowActionHandler.AddHandler(g_pGame->Actions().attack1_xi, &CPickAndThrowWeapon::OnActionAttack);	//throw
	...
	switch (m_state)
	{
		...
		case eST_CHARGED_THROW_POST_RELEASE:
		{
			...
			if (m_timePassed >= GetThrowParams().timeToFreeAtThrowing || m_timePassed>=m_animDuration)
			{
				if (pOwner)
				{
					pOwner->NotifyObjectGrabbed( false, m_objectId, IsNPC() );
				}
				if (IsNPC())
				{
...
void CPickAndThrowWeapon::ThrowObject()
{
	...
	if (pOwnerPlayer != NULL)
	{
		...
		speed = speed - (cry_fabsf(throwDir.z) * kSpeedReductionVertical * speed); // reduce throw speed slightly at sharp angles (both up + down)
