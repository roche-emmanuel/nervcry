	...
	if (pCharacterInstance)
	{
		...
		if (hitInfo.partId >= PHYSICS_ATTACHMENT_BASE_ID)
		{
			...
			if ((pAttachment != NULL) && GetDestructibleAttachment(HashString(pAttachment->GetName()), bodyPart))
			{
				...
				effectAttachmentBone = pICharacterModelSkeleton->GetJointNameByID(pAttachment->GetJointID());
				hitPartName = pAttachment->GetName();
			}
		}
		else if (hitInfo.partId >= 0)
		{
			const char* boneName = pICharacterModelSkeleton->GetJointNameByID(pSkeletonPose->getBonePhysParentOrSelfIndex(hitInfo.partId));
...
void CBodyDestructibilityProfile::ProcessDestructiblesOnExplosion( IEntity& characterEntity, CBodyDestrutibilityInstance& instance, const HitInfo& hitInfo, const float previousHealth, const float newHealth )
{
	...
	if (pCharacterInstance)
	{
		...
		ICharacterModelSkeleton* pICharacterModelSkeleton = pCharacterInstance->GetICharacterModel()->GetICharacterModelSkeleton();
		CRY_ASSERT(pICharacterModelSkeleton);
		...
		while((attachmentIdx < destructibleAttachments) && (destroyedBodyParts < maxDestructiblesByExplosion))
		{
			...
			if ((pPartStatus != NULL) && !pPartStatus->IsDestroyed())
			{
				...
				if (pPartStatus->IsDestroyed() && instance.CanTriggerEvent(destructionEventId) && CheckAgainstScriptCondition(characterEntity, destructionEventId))
				{
					...
					const char *effectAttachmentBone = pAttachment ? pICharacterModelSkeleton->GetJointNameByID(pAttachment->GetJointID()) : "";
	...
	if (pCharacterInstance)
	{
		...
		if (shouldGib)
		{
			...
			if (instance.CanTriggerEvent(destructionEventId))
			{
				...
				const QuatT& boneLocation = pSkeletonPose->GetAbsJointByID(pICharacterModelSkeleton->GetJointIDByName(m_gibDeath.bone.c_str()));
...
void CBodyDestructibilityProfile::ProcessDeathByHit( IEntity& characterEntity, ICharacterInstance& characterInstance, CBodyDestrutibilityInstance& instance, const HitInfo& hitInfo )
{
	...
	ICharacterModelSkeleton* pICharacterModelSkeleton = characterInstance.GetICharacterModel()->GetICharacterModelSkeleton();
	CRY_ASSERT(pICharacterModelSkeleton);
