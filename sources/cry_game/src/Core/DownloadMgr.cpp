...
#if defined(XENON)
static const char *k_platformPrefix="360";
#elif defined(PS3)
static const char *k_platformPrefix="ps3";
#else
...
{
	const char *gameTitle="Crysis3";
	const char *version="1.0";

	// TODO - hook up current version number if we're bothered
#if defined(PS3)
	ioAgentStr.Format("%sPS3/%s", gameTitle, version);
#elif defined(XENON)
	ioAgentStr.Format("%sXBOX/%s", gameTitle, version);
#else
	if (gEnv->IsDedicated())
	{
