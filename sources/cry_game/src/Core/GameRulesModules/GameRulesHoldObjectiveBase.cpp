...
{
	if (m_bExpectingMovieStart && (movieEvent == IMovieListener::MOVIE_EVENT_START))
	{
		CryLog("CGameRulesHoldObjectiveBase::OnMovieEvent() expected trackview has started - %s; setting NO_MP_SYNCING_NEEDED flag on it", pAnimSequence->GetName());
		m_pStartingAnimSequence = pAnimSequence;

		pAnimSequence->SetFlags(IAnimSequence::NO_MP_SYNCING_NEEDED);
