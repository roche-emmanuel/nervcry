...
// Soft-coding: 
IMPLEMENT_TYPE(CSceneBlurGameEffect); // Provides the factory creation and access to reflection
		...
		case eKI_NP_1:
			{
				...
				if(pEffect == NULL)
				{
					pEffect = CREATE_GAME_FX_SOFT_CODE_INSTANCE(CSceneBlurGameEffect);
					if(pEffect)
					{
						pEffect->Initialise();
