...
public:
	...
	bool EarlyExitStartTransition()
	{
		...
		IF_UNLIKELY (!(m_flags & OnLastAnim))
			return false;
...
private:
	...
	bool FragmentExistsInDatabase(const SAnimationContext& context, const IAnimationDatabase& database) const
	{
		...
		const uint32 optionCount = database.FindBestMatchingTag(m_fragmentID, fragTagState);
...
{
	const float minimumSpeedForRunStopSquared = cry_sqr(3.5f);
...
float CCornerSmoother::CalculateDirSmoothTime(const SCurveSettings& settings, const float desiredSpeed, const float distToMoveTarget2D, const float endOfPathDistance)
{
	...
	const float speedFactor = clamp((desiredSpeed - settings.walkSpeed)/(settings.runSpeed - settings.walkSpeed), 0 , 1);
	return clamp(distToEndOfCornerSmoothing*(1.0f/STOP_CORNER_SMOOTHING_DISTANCE), 0.0001f, 1.0f) * LERP(settings.walkSmoothTime, settings.runSmoothTime, speedFactor);
}
	...
	if (!atEndOfPath && !hasLockedBodyTarget)
	{
		...
		if (bodyAngleDifference > FLT_EPSILON)
		{
			...
			if (IsRunning())
			{
				...
				float factor = clamp(- cos(bodyAngleDifference), 0 , 1);
...
void CCornerSmoother::PredictSmoothTurn(const SPredictionSettings& settings, SPrediction*const pPrediction)
{
	...
	while(i < MAX_SAMPLES-1) // '-1' to leave one corner entry for the moveTarget at the end
	{
		...
		const float factor = clamp(- cos(angleDifference), 0 , 1);
		...
		const float clampedAngleDiff = clamp(angleDelta, -maxAngleDelta, maxAngleDelta);
