...
void CLargeObjectInteraction::AdjustObjectTrajectory( float frameTime )
{
	...
	if (pObjectPhysics && pObjectPhysics->GetStatus(&objectDynamicStatus))
	{
		...
		if (pTargetInfo)
		{
			...
			const float totalDeviationAngle = RAD2DEG(cry_acosf(currentDirection.Dot(goalDirection)));
			CRY_ASSERT(totalDeviationAngle >= 0.0f);

			const float frameDeviationAngle = clamp(20.0f * frameTime, 0.0f, totalDeviationAngle);
