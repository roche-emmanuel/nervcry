...
public:
	...
	virtual EStatus UpdatePending(float timePassed)
	{
		...
		if( m_rootScope && !m_isForced )
		{
			...
			const uint32 numMatchingTags = m_rootScope->GetDatabase().FindBestMatchingTag( m_fragmentID, fragTagState, &matchedTagState );
...
struct CHitDeathReactions::SPredFindValidReaction : public std::unary_function<bool, const SReactionParams&>
{
	...
	bool operator ()(const SReactionParams& reactionParams) const
	{
		...
		if( bFound && gEnv->bMultiplayer && reactionParams.mannequinData.actionType == SReactionParams::SMannequinData::EActionType_Coop )
		{
			...
			{
				SFragTagState matchedTagState; 
				const uint32 numAnimsPotential = piOptionalAnimationADB->FindBestMatchingTag( PlayerMannequin.fragmentIDs.hitDeath, fragTagState, &matchedTagState );
...
void CHitDeathReactions::DrawDebugInfo()
{
	...
	if (g_pGameCVars->g_hitDeathReactions_enable)
	{
		...
		if (IsInReaction())
		{
			...
			{
				string sAnimName = GetCurrentAGReactionAnimName();
				sMsg2 += string().Format("\nExecuting Mannequin reaction: Tags: %d", m_pCurrentReactionParams->mannequinData.tagState.GetBits() );
			}
...
{
	const CTagDefinition* pTagDefinition = gEnv->pGame->GetIGameFramework()->GetMannequinInterface().GetAnimationDatabaseManager().LoadTagDefs( "Animations/Mannequin/ADB/hitDeathTags.xml" );
