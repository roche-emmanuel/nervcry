...
bool CSpectatorFollowCameraMode::UpdateView( const CPlayer& clientPlayer, SViewParams& viewParams, float frameTime )
{
	...
	if(followCameraSettings.m_flags & SFollowCameraSettings::eFCF_AllowOrbitPitch)
	{
		...
		m_orbitX = clamp(m_orbitX, g_pGameCVars->g_spectate_follow_orbitMinPitchRadians, g_pGameCVars->g_spectate_follow_orbitMaxPitchRadians);
	}
	else
	{
		m_orbitX = 0.f;
...
void CSpectatorFollowCameraMode::ApplyEffects( const CPlayer& clientPlayer, const SViewParams& viewParams, float focalDist, float minFocalRange )
{
	...
	if( prevMode != specMode )
	{
		...
		{
			// Activate 
			g_pGame->GetRecordingSystem()->GetKillCamGameEffect().SetCurrentMode(CKillCamGameEffect::eKCEM_KillerCam);
			g_pGame->GetRecordingSystem()->GetKillCamGameEffect().SetActiveIfCurrentMode(CKillCamGameEffect::eKCEM_KillerCam,true);
	...
	case CActor::eASM_Killer:
		{
			...
			g_pGame->GetRecordingSystem()->GetKillCamGameEffect().SetActiveIfCurrentMode(CKillCamGameEffect::eKCEM_KillerCam,false);
		}
		break;
	}
}
...
bool CDeathCameraModeSinglePlayer::UpdateView(const CPlayer& clientPlayer, SViewParams& viewParams, float frameTime)
{
	...
	if (m_params.dof_enable)
	{
		...
		m_fFocusDistance += static_cast<float>(__fsel(fDelta, 1.0f, -1.0f) * min(cry_fabsf(fDelta), (m_params.dofDistanceSpeed * frameTime)));

		// Approximate range
		fDelta = fDesiredFocusRange - m_fFocusRange;
		m_fFocusRange += static_cast<float>(__fsel(fDelta, 1.0f, -1.0f) * min(cry_fabsf(fDelta), (m_params.dofRangeSpeed * frameTime)));
