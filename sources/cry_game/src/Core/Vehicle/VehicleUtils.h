...
{
	value += clamp(target - value, -amount, +amount);
}
...
public:
	...
	void Draw(CTimeSmoothedValue& value, float x, float y, float width, float height)
	{
		...
		// Draw the main bar
		const float valueFraction = clamp((value.m_value-m_minValue)/(m_maxValue-m_minValue), 0.f, 1.f);
		...
		const float yValueFiltered = y - height * clamp((value.m_filteredValue - m_minValue)/(m_maxValue-m_minValue), 0.f, 1.f);
