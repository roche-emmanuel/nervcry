		...
		case eIUS_General:
			{
				...
				if (needsDofUpdate)
				{
					...
					s_dofValue = clamp(newDofValue, 0.0f, 1.0f);
...
void CWeapon::GetFPOffset(QuatT &offset) const
{
	if (m_zm)
	{
		...
		else if (m_zm->IsZoomingInOrOut())
		{
			...
			ironSightWeight = clamp(ironSightWeight, 0.0f, 1.0f);

			ironSightWeight = cry_sinf(gf_PI * 0.5f * ironSightWeight);
...
//			ISkeletonPose* pSkelPose = pCharacter->GetISkeletonPose();
			...
			if(pICharacterModelSkeleton)
			{
				...
				const int16 jointId = pICharacterModelSkeleton->GetJointIDByName(jointName.c_str());
