...
// ===========================================================================
	...
	namespace Debug
	{
		...
		{
			return LERP(badColor, goodColor, clamp(sqrtf(sample.maximumSpeedSq)/6.0f, 0.0f, 1.0f));
		}
	...
	public:
		...
		bool IsValidFor(const SState& state) const OVERRIDE
		{
			...
			if (m_lastAlpha > 0.0f)
			{
				...
				{
					CORNERSMOOTHER_LOG("Plan Invalid: Movement direction too different(%f > %f)", RAD2DEG(acosf(clamp(cosAngle, -1.0f, 1.0f))), RAD2DEG(TAKECORNER_MAX_MOVEMENTDIRECTION_ANGLE_DIVERGENCE));
					return false;
				}
		...
		EPlanExecutionResult Execute(const SState& state, SOutput& output) OVERRIDE
		{
			...
			if (wasMoving)
			{
				...
				const float newDesiredSpeed = clamp(state.desiredSpeed, minimumSpeed, maximumSpeed);
