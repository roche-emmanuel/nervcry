...
Vec3 CSingle::GetProbableHit(float maxRayLength, bool *pbHit, ray_hit *pHit) const
{
	...
	if (pAmmoParams)
	{
		...
		rayLength = clamp(maxSpeed * pAmmoParams->lifetime, 5.0f, rayLength);
	}
...
int CSingle::GetDamage() const
{
	...
	const bool canOversharge = m_pWeapon->GetSharedItemParams()->params.can_overcharge;
	if (canOversharge)
	{
...
float CSingle::GetProjectileFiringAngle(float v, float g, float x, float y)
{
	...
	float d = cry_sqrtf(max(0.0f,powf(v,4)-2*g*y*( v * v )- (g * g) * (x *x)));
	if(d>=0)
	{
		a= (v * v)-g*y;
		if (a-d>0) {
			t=cry_sqrtf(2*(a-d)/(g * g));
