...
void CPlayerStateGround::OnPrePhysicsUpdate( CPlayer& player, const SActorFrameMovementParams &movement, float frameTime, const bool isHeavyWeapon, const bool isPlayer )
{
	...
	else
	{
		...
		if (isPlayer)
		{
			...
			alignment *= (float)__fsel(-fabsf(stats.onGround), LERP(0.7f, 1.0f, 1.0f - clamp(modifiedSlopeNormal.z * 100.0f, 0.0f, 1.0f)), 1.0f);
...
#if defined(PS3)
							const float iconColor[4] = {0.6f, 0.6f, 1.f, 1.0f};
