...
public:
	...
	virtual void OnMovieEvent(IMovieListener::EMovieEvent event, IAnimSequence* pSequence) 
	{
		...
		if (event == IMovieListener::MOVIE_EVENT_STOP || event == IMovieListener::MOVIE_EVENT_ABORTED)
		{
			ActivateOutput(&m_ActInfo, EOP_Finished, true);
...
public:
	...
	void SetupIntroStart(const EntityId cameraPoint, const float introDuration, const char* pIntroSound)
	{
		...
		g_pGame->GetRecordingSystem()->GetKillCamGameEffect().SetCurrentMode(CKillCamGameEffect::eKCEM_IntroCam);
		g_pGame->GetRecordingSystem()->GetKillCamGameEffect().SetActiveIfCurrentMode(CKillCamGameEffect::eKCEM_IntroCam,true);
					...
					{	
						...
						{ 	
							...
							if(!pMovieSystem->IsPlaying(pSequence))
							{
								...
								m_pSequence->SetFlags(IAnimSequence::NO_MP_SYNCING_NEEDED);
		...
		if (event == eFE_Activate)
		{
			...
			if (pICVar != 0)
			{
				...
				if(pICVar->GetType()==CVAR_FLOAT)
				{
					...
					ActivateOutput(pActInfo, BOOLVALUE, cry_fabsf(fval)>FLT_EPSILON);
