...
void CVehicleMovementArcadeWheeled::PostUpdateView(SViewParams &viewParams, EntityId playerId)
{
	...
	const float speedNorm = clamp(speed*invTopSpeed, 0.f, 1.f);
	...
	float angle = clamp(-linearAdjustment*va->rotate + cent*va->rotateCent, -va->maxRotate, +va->maxRotate);
	float lateral = clamp(linearAdjustment*va->lateralDisp + cent*va->lateralCent, -va->maxLateralDisp, +va->maxLateralDisp);
	float forward = clamp(-speedNorm*va->forwardResponse, -va->maxForwardDisp, +va->maxForwardDisp);
	...
	m_handling.viewOffset.z += (va->suspensionAmp*clamp(compression*va->suspensionResponse,-1.f, +1.f) - m_handling.viewOffset.z)*approxOneExp(va->suspensionSharpness*dt);
...
void CVehicleMovementArcadeWheeled::UpdateSounds(const float deltaTime)
{
	...
	if (m_isEnginePowered && !m_isEngineGoingOff)
	{
		...
		m_rpmScale = clamp(m_gears.curRpm + m_pSharedParams->gears.gearOscillationAmp*oscillation, 0.f, 1.f);
		SetSoundParam(eSID_Run, "rpm_scale", m_rpmScale);
		SetSoundParam(eSID_Run, "rpm_load", m_gears.rpmLoad);
		SetSoundParam(eSID_Run, "clutch", clamp(m_gears.modulation + m_pSharedParams->gears.gearOscillationAmp2*oscillation, 0.f, 1.f));
		...
		if (DoGearSound() && m_gears.doGearChange)
		{
			...
			SetSoundParam(eSID_Gear, "rpm_scale", m_rpmScale);
			SetSoundParam(eSID_Gear, "rpm_load", m_gears.rpmLoad);
		}
	
		if (m_action.bHandBrake)
			StopSound(eSID_Boost, false);
	...
	m_movementInfo.skidValue = clamp(m_movementInfo.skidValue, 0.f, 1.f);

	SetSoundParam(eSID_Run, "slip", m_movementInfo.skidValue);
	...
	{ 
		if (m_compressionMax > soundParams->bumpMinSusp)
		{
			...
			if (ISound* pSound = PlaySound(eSID_Bump))
			{
				...
				pSound->SetParam("intensity", min(1.f, soundParams->bumpIntensityMult*m_compressionMax/soundParams->bumpMinSusp), false);
				m_lastBump = 0;
			}      
		}            
	}   
...
void CVehicleMovementArcadeWheeled::InternalPhysicsTick(float dt)
{
	...
	//=============================================
	float collisionThrottleScale = 0.2f*clamp(1.f-3.f*(fsgnf(m_movementAction.power)*m_collisionNorm.dot(yAxis)/physStatus->mass), 0.5f, 1.f) + 0.8f;
...
// forcing an early store.
	...
	#define F2I(x) (int32)(x)
	#define TMPINT volatile int32
#else
	#define F2I(x) (int)(x)
...
void CVehicleMovementArcadeWheeled::TickGears(float dt, float averageWheelSpeed, float throttle, float forwardSpeed, float contact)
{
	...
	if (m_isEnginePowered)
	{
		...
		const float contactModifier = 1.f/clamp(contact+0.3f,0.4f,1.3f);
		...
		m_gears.targetRpm = m_gears.modulation*clamp(rpm, 0.1f, 1.3f);
		...
		m_gears.rpmLoad = clamp(m_gears.rpmLoad + (totalRpmLoad - m_gears.rpmLoad) * approxOneExp(dt*loadInterpSpeed), -1.f, +1.f);
	}
}
