...
void CLocalPlayerComponent::StartFreeFallDeath()
{
	...
	currentLook.z = cry_tanf(DEG2RAD(g_pGameCVars->pl_freeFallDeath_cameraAngle));
...
void CLocalPlayerComponent::AdjustTorsoAimDir(float fFrameTime, Vec3 &aimDir)
{
	...
	if (g_pGameCVars->pl_swimAlignArmsToSurface && m_rPlayer.IsSwimming() && (m_rPlayer.m_playerStateSwim_WaterTestProxy.GetRelativeWaterLevel() > MIN_FLATTEN_LEVEL))
	{
		...
		newElevLimit = LERP(gf_PI * 0.5f, TARGET_FLATTEN_ELEV, clamp(newElevLimit, 0.0f, 1.0f));
	}

	float limitDelta = LIMIT_CHANGE_RATE * fFrameTime;
	float limitDiff	 = newElevLimit - m_stapElevLimit;
	float smoothedLimit = (float) __fsel(cry_fabsf(limitDiff) - limitDelta, m_stapElevLimit + (fsgnf(limitDiff) * limitDelta), newElevLimit);
	...
	if (smoothedLimit < HALF_PI)
	{
		...
		if (xy > 0.001f)
		{
			...
			elev = asin_tpl(clamp(aimDir.z, -1.f, +1.f));
		}
		else
		{
			yaw = 0.f;
...
void CLocalPlayerComponent::UpdateFPIKTorso(float fFrameTime, IItem * pCurrentItem, const Vec3& cameraPosition)
{
	...
	if (pCharacter != 0)
	{
		...
		CIKTorsoAim_Helper::SIKTorsoParams IKParams(pCharacter, pCharacterShadow, aimDir, torsoOffset, cameraPosition, ShouldUpdateTranslationPinning(), needsPositionAdjust);
