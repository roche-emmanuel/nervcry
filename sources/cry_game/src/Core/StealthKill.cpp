...
CStealthKill::ECanExecuteOn CStealthKill::CanExecuteOn(CActor* pTarget, bool checkExtendedRange) const
{
	...
	if (isValidTarget && pStealthKillParams)
	{
		...
		if(cry_sqr(v3PlayerToTarget.z) < cry_sqr(pStealthKillParams->maxHeightDiff))
		{
			...
			if (distSquared >= cry_sqr(maxDistance))
			{
			...
			if (checkExtendedRange && g_pGameCVars->pl_stealthKill_usePhysicsCheck)
			{
				...
				if (distSquared >= cry_sqr(maxNormalDistance))
				{
	...
	if(pTarget)
	{
		...
		{	
			...
			if (pStealthKillParams)
			{
				...
				int boneId = pCharacter ? pCharacter->GetICharacterModel()->GetICharacterModelSkeleton()->GetJointIDByName("Spine04") : -1;
				if (boneId != -1)
				{
