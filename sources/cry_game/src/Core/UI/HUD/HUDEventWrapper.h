...
struct SHUDEventWrapper
{
	...
	static void OnRoundEnd( const int winner, const bool clientScoreIsTop, const char* victoryDescMessage, const char* victoryMessage, const TAudioSignalID roundEndMusicSignal, const EAnnouncementID announcement );
	static void OnSuddenDeath( void );
	static void OnGameEnd(const int teamOrPlayerId, const bool clientScoreIsTop, const EGameOverReason reason, const char* message, const ESVC_DrawResolution drawResolution, const TAudioSignalID musicSignal, const EAnnouncementID announcement);
