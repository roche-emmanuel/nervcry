...
#if defined(XENON)
	#define	MENU_LIGHT_COLOR_SCALE					0.4f
	#define MENU_LIGHT_AMBIENT_ALPHA_SCALE	1.0f
#elif defined(PS3)
	#define	MENU_LIGHT_COLOR_SCALE					0.9f
	#define MENU_LIGHT_AMBIENT_ALPHA_SCALE	0.375f
#else
...
bool CMenuRender3DModelMgr::HasCharacterStreamedIn(SRenderSingleEntityData& rd, ICharacterInstance* pCharacter)
{
	...
	if (ICharacterModel* pCharModel = pCharacter->GetICharacterModel())
	{
		if (!pCharModel->GetIRenderMesh())
		{
			MR3MM_TRACE("[MR3MM] (%p) Missing character model render mesh '%s'", &rd, pCharModel->GetModelFilePath());
			bHasStreamed = false;
		}
	}
