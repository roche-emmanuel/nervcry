			...
			if (m_recordedData.m_corpses.size() < m_recordedData.m_corpses.max_size())
			{
				...
				if(IEntity* pCorpseEntity = gEnv->pEntitySystem->GetEntity(pCorpsePacket->corpseId))
				{
					if(ICharacterInstance* pChar = pCorpseEntity->GetCharacter(0))
					{
						...
						ICharacterModelSkeleton* pICharacterModelSkeleton = pChar->GetICharacterModel()->GetICharacterModelSkeleton();
						if(pPose && pICharacterModelSkeleton)
						{
						  STrackedCorpse corpse;
						  CRY_ASSERT(pICharacterModelSkeleton->GetJointCount() <= STrackedCorpse::k_maxJoints);
							corpse.m_corpseId = pCorpsePacket->corpseId;
							corpse.m_numJoints = min((int32) pICharacterModelSkeleton->GetJointCount(), static_cast<int32>(STrackedCorpse::k_maxJoints));
							...
							{
								const int16 jointIdx = pICharacterModelSkeleton->GetJointIDByCRC32(*it);
								if(jointIdx!=-1)
							...
							for (int32 i=0; i<corpse.m_numJoints; i++)
							{
								if(s_ADIKJoints[i])
								{
									...
									rel.t.x = 0.f;
									const int32 parent = pICharacterModelSkeleton->GetJointParentIDByID(i);
									if(parent<0)
									{
...
void CRecordingSystem::FillOutFPCamera(SRecording_FPChar *cam, float playbacktime)
{
	...
	if (loweroffset >= 0)
	{
		...
		float lerpval = clamp((playbacktime - loweroffsettime) / timediff, 0.f, 1.f);
...
void CRecordingSystem::UpdateView(SViewParams &viewParams)
{
	...
	if (m_cameraSmoothing && g_pGameCVars->kc_smoothing > 0)
	{
		...
		if(lenSqr>FLT_EPSILON)
		{
			...
			const float invlen = cry_isqrtf(lenSqr);
...
#undef SetFlags
	...
	if (pReplayEntity)
	{
		...
		if (updatePosition)
		{
			...
			if (pReplayEntity->IsHidden() && !(pReplayActor->m_flags & eRAF_HaveSpawnedMyCorpse))
			{
...
bool CRecordingSystem::OnInputEvent(const SInputEvent &rInputEvent)
{
	...
	if (rInputEvent.deviceId == eDI_Keyboard && rInputEvent.state == eIS_Pressed)
	{
				...
				if (event == SOUNDSYSTEM_EVENT_ON_START)
				{
					...
					if (!m_excludedSoundEffects.count(nameHash))
					{
						...
						if (playSound.szName)
						{
							...
							if (pExtended)
								playSound.entityId = pExtended->GetSoundProxyEntityID();
	...
	if (m_killerReplayGunId != 0)
	{
		...
		for (itReplaySound = m_replaySounds.begin(); itReplaySound != m_replaySounds.end(); ++itReplaySound)
		{
			...
			if (pSound)
			{
				...
				if (pExtended)
				{
					EntityId entityId = pExtended->GetSoundProxyEntityID();
...
void CRecordingSystem::ApplyPlaySound(const SRecording_PlaySound* pPlaySound, float time)
{
	...
	if (itReplaySound == m_replaySounds.end())
	{
		...
		if (pSound)
		{
			...
			if (pEntity)
			{
				IEntitySoundProxyPtr pSoundProxy = crycomponent_cast<IEntitySoundProxyPtr>(pEntity->CreateProxy(ENTITY_PROXY_SOUND));
				CRY_ASSERT_MESSAGE(pSoundProxy.get(), "Failed to create sound proxy");
				if(pSoundProxy)
				{
...
void CRecordingSystem::GetVictimAimTarget( Vec3 &aimTarget, const Vec3 &localPosition, const Vec3 &remotePosition, float lerpValue )
{
	...
	dotProduct = clamp(dotProduct, -1.f, 1.f);
