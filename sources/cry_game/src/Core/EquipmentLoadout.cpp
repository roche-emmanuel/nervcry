	...
	if (CFrontEndModelCache::IsAllowed3dFrontEndAssets())
	{
		...
		if (primaryItemName && primaryItemName[0])
		{
			...
			if (pItemSharedParams)
			{
				...
				const char * itemHoldPoseName = pItemSharedParams->params.pose.c_str();
				// TODO: Implement game code side of 3d models in front end
				//render3DModelMgr->SetAnim(string().Format("_stand_tac_idle_%s_3p_01", itemHoldPoseName).c_str(), characterModelIndex, SPEED_MULTIPLIER, 0);
